...
class IslandMaxFlowGraph(object):
    ...
    def copy(self) -> IslandMaxFlowGraph:
        ...
        clone = IslandMaxFlowGraph([n.copy() for n in self.root_flow_nodes_no_neut], [n.copy() for n in self.enemy_backfill_nodes_inc_neut], [e.copy() for e in self.enemy_backfill_neut_dump_edges])
...
class ArmyFlowExpander(object):
    ...
    def _build_max_flow_min_cost_flow_nodes(
            ...
            for targetNodeId, targetFlowAmount in targets.items():
                if isThroughput:
                    if targetNodeId != -nodeId:
                        raise AssertionError(f'input node flowed to something other than output node...?  {sourceNode} -{targetFlowAmount}> {targetNodeId}')
                    sourceNode.army_flow_received = targetFlowAmount
                    continue
                if targetFlowAmount == 0:
                    # raise AssertionError(f'wut? Connection, but zero flow between  {sourceNode} -{targetFlowAmount}> {targetNodeId}')
                    # logbook.info(f'wut? Connection, but zero flow between  {sourceNode} -{targetFlowAmount}> {targetNodeId}')
                    continue
                ...
                if self.log_debug:
                    logbook.info(f'FOUND INC NEUT FLOW EDGE {sourceNode} -{targetFlowAmount}> {targetNode}')
        ...
        for nodeId, targets in noNeutFlowDict.items():
            ...
            for targetNodeId, targetFlowAmount in targets.items():
                ...
                if targetNodeId in self.nxGraphDataNoNeut.neutral_sinks and sourceNode.island is targetGeneralIsland:
                    # raise AssertionError(f'wut? shouldnt have backfill neut edges here? No demand allowed on neuts?   {sourceNode} -{targetFlowAmount}> {targetNode}')
                    edge = IslandFlowEdge(targetNode, targetFlowAmount)
                ...
                sourceNode.set_flow_to(targetNode, targetFlowAmount)
                if self.log_debug:
                    logbook.info(f'FOUND NO NEUT FLOW EDGE {sourceNode} -{targetFlowAmount}> {targetNode}')
        ...
        flowGraph = IslandMaxFlowGraph(finalRootNoNeutFlowNodes, finalRootFlowNodes, enemyBackfillNoNeutFlowNodes, enemyBackfillFlowNodes, backfillNeutEdges, backfillNeutNoNeutEdges)
    ...
    def find_flow_plans(
        ...
        while q:
            ...
            if True:
                ...
                (
                    ...
                    tileIterCount,
                    turnsLeft
                ) = self._execute_island_tile_gather_loop_and_record_options(
                    uncappedTargetIslandArmy,
                    capValue,
                    dumpIterCount,
                    econValue,
        ...
        for (targetIsland, source), planOptionsByTurns in opts.items():
            for turns, (rootNodes, econValue, otherTurns, armyRemainingInIncompleteSource, incompleteTarget, incompleteSource, unusedSourceArmy, gathedArmySum) in planOptionsByTurns.items():
                ...
                plan = self._build_flow_expansion_option(islands, targetIsland, source, rootNodes, econValue, armyRemainingInIncompleteSource, turns, targetIslands, ourIslands, incompleteTarget, incompleteSource, unusedSourceArmy,
                                                         gathedArmySum)
                output.append(plan)
    ...
    def _execute_island_tile_gather_loop_and_record_options(
            self,
            uncappedTargetIslandArmy,
            capValue,
            dumpIterCount,
    ...
    def _queue_next_targets_and_next_friendlies(
        ...
        for (nextTargetIsland, nextTargetFromNode) in nextTargets.items():
            ...
            # add
            newArmyToCap = uncappedTargetIslandArmy
    ...
    def _calculate_next_target_and_queue_next_friendlies(
        ...
        dumpIterCount, newArmyToCap, newEconValue, newTurnsLeft, newFrLeftoverArmy, newFriendlyTileLeftoverIdx, tileIterCount = self._calculate_next_target_values(
            capValue,
            dumpIterCount,
            friendlyUncalculated,
            newArmyToCap,
    ...
    def _calculate_next_target_values(
        ...
        (
            ...
            turns,
            newTurnsLeft)
        return dumpIterCount, newArmyToCap, newEconValue, newTurnsLeft, newFrLeftoverArmy, newFriendlyTileLeftoverIdx, tileIterCount
    ...
    def live_render_capture_stuff(self, islandBuilder: TileIslandBuilder, capping: TileSet, gathing: TileSet, incompleteSource: TileIsland, incompleteTarget: TileIsland, sourceUnused: int | None = None, extraInfo: str | None = None):
    ...
    @staticmethod
    def add_flow_graph_to_view_info(flowGraph: IslandMaxFlowGraph, viewInfo: ViewInfo, noNeut: bool = True, withNeut: bool = False, showBackfillNeut: bool = False):
        q: typing.Deque[IslandFlowNode] = deque()
        if noNeut:
            for flowSource in flowGraph.root_flow_nodes_no_neut:
                q.append(flowSource)
            while q:
            ...
            for flowSource in flowGraph.enemy_backfill_nodes_no_neut:
                q.append(flowSource)
            while q:
                flowNode: IslandFlowNode = q.popleft()
        ...
        if withNeut:
            for flowSource in flowGraph.root_flow_nodes_inc_neut:
                q.append(flowSource)
            while q:
                flowNode: IslandFlowNode = q.popleft()
                ...
                for destinationEdge in flowNode.flow_to:
                    ...
                    viewInfo.draw_diagonal_arrow_between_xy(sourceX, sourceY, destX, destY, label=f'{destinationEdge.edge_army}', color=Colors.DARK_PURPLE)
                    q.append(destinationEdge.target_flow_node)

            for flowSource in flowGraph.enemy_backfill_nodes_inc_neut:
                q.append(flowSource)
            while q:
                flowNode: IslandFlowNode = q.popleft()
                ...
                for destinationEdge in flowNode.flow_to:
                    ...
                    viewInfo.draw_diagonal_arrow_between_xy(sourceX, sourceY, destX, destY, label=f'{destinationEdge.edge_army}', color=Colors.LIGHT_BLUE, alpha=155)
                    q.append(destinationEdge.target_flow_node)
    ...
    def _build_island_flow_precursor_nx_data(self, islands: TileIslandBuilder, useNeutralFlow: bool) -> NxFlowGraphData:
        ...
        targetGeneralIsland: TileIsland = islands.tile_island_lookup.raw[self.enemyGeneral.tile_index]
        cumulativeDemand, demands = self._determine_initial_demands_and_split_input_output_nodes(graph, islands, ourSet, targetSet)
        neutSinks = set()
        capacityLookup = {}
        ...
        for island in islands.all_tile_islands:
            ...
            for movableIsland in island.border_islands:
                ...
                graph.add_edge(-island.unique_id, movableIsland.unique_id, weight=1, capacity=edgeCapacity)
        ...
        for neutSinkId in neutSinks:
            ...
            graph.add_edge(-targetGeneralIsland.unique_id, neutSinkId, weight=0, capacity=capacity)

        return NxFlowGraphData(graph, neutSinks, demands, cumulativeDemand)

    def _determine_initial_demands_and_split_input_output_nodes(self, graph, islands, ourSet, targetSet):
        demands = {}
        cumulativeDemand = 0
    ...
    def _get_a_star_priority_val(self, capValue: int, newEconValue: float, newTurnsUsed: int, newFrLeftoverArmy: int, newFriendlyTileLeftoverIdx: int, friendlyUncalculated: TileIsland, searchTurns: int, tgTiles: typing.Deque[int]) -> float:
        """
