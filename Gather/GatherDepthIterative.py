...
def _knapsack_gather_iteration(
        turns: int,
...
def _get_sub_knapsack_gather(
    ...
    gatheredArmy, maxPaths = _knapsack_gather_iteration(remainingTurns, valuePerTurnPathPerTilePerDistance, logList=logEntries if shouldLog else None)

    return int(round(gatheredArmy)), maxPaths


def _get_single_line_iterative_starter(
        fullTurns,
        ignoreStartTile,
        incrementBackward,
        logEntries,
...
def _build_next_level_start_dict(
        newPaths: typing.List[Path],
        startTilesDict: typing.Dict[Tile, typing.Tuple[typing.Any, int]],
        searchingPlayer: int,
        remainingTurns: int,
...
def _add_tree_nodes_to_start_tiles_dict_recurse(
    ...
    for node in rootNode.children:
        added += _add_tree_nodes_to_start_tiles_dict_recurse(node, startTilesDict, searchingPlayer, remainingTurns, baseCaseFunc, dist=dist + 1)
    #
...
def _knapsack_levels_gather_recurse(
    ...
    for turnsToTry in turnCombos:
        ...
        while calculatedLeftOverTurns < remainingTurns:
            ...
            turnsLeft, newStartTilesDict = _build_next_level_start_dict(
                newPaths,
                newStartTilesDict,
                searchingPlayer,
        ...
        while lastPrunedTo < fullTurns:
            ...
            for path in newPaths:
                ...
                if rootNode is not None:
                    ...
                    _add_tree_nodes_to_start_tiles_dict_recurse(
                        rootNode,
                        newStartTilesDict,
                        searchingPlayer,
                        calculatedLeftOverTurns,
...
def knapsack_depth_gather(
    ...
    @return:
    """
    totalValue, usedTurns, rootNodes = knapsack_depth_gather_with_values(
        map=map,
...
def knapsack_depth_gather_with_values(
        map: MapBase,
        startTiles,
        turns: int,
        targetArmy=None,
    ...
    if priorityFunc is None:
        ...
        def default_priority_func(nextTile, currentPriorityObject):
            ...
            realDist += 1
            depthDist += 1
            depthHeuristicDist = depthDist
            if depthPriorityMap is not None:
                depthHeuristicDist += depthPriorityMap.raw[nextTile.tile_index]
            prioObj = (
                depthHeuristicDist,
                depthDist,
                realDist,
    ...
    if baseCaseFunc is None:
        ...
        def default_base_case_func(tile, startingDist):
            ...
            if distPriorityMap is not None:
                # passes 4 more Defense tests...?
                initialDistance = distPriorityMap.raw[tile.tile_index]
            depthHeuristicDist = startingDist
            if depthPriorityMap is not None:
                depthHeuristicDist += depthPriorityMap.raw[tile.tile_index]
            armyNegSum = 0
            gathNegSum = 0
