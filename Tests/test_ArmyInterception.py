...
class ArmyInterceptionTests(TestBase):
    ...
    def test_should_block_to_the_left__with_move_half(self):
        ...
        simHost.queue_player_moves_str(enemyGeneral.player, '14,8->13,8->13,14->12,14->12,16')
        #proof
        # simHost.queue_player_moves_str(general.player, '15,9->14,9z->13,9')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
    ...
    def test_should_assume_army_can_cap_tiles_away_from_general(self):
        ...
        self.skipTest('no asserts yet, bot does dumb shit wobbling back and forth')
    ...
    def test_should_defend_effectively_when_not_safe_to_dive(self):
        ...
        for path, minTileDiff in [
            ...
            with self.subTest(minTileDiff = minTileDiff, path=path):
                ...
                simHost.queue_player_moves_str(enemyGeneral.player, path)
                simHost.sim.ignore_illegal_moves = True
                simHost.queue_player_moves_str(general.player, '9,7->9,8')  # bypass the king kill dive that messes up the test
                #proof
                # simHost.queue_player_moves_str(general.player, '9,8->9,9->10,9->10,10')
    ...
    def test_should_intercept_one_late_at_midpoint_choke(self):
        ...
        value, turns, bestOpt = self.get_best_intercept_option_path_values(plan)
        self.assertEqual(1, bestOpt.length)
        self.assertEqual(map.GetTile(2, 6), bestOpt.start.next.tile)
    ...
    def test_should_cap_tiles_not_draw_7_from_general(self):
        ...
        for path, minEcon in [
            ('14,7->13,7->13,4', 4),  # only 4 econ because we do have to waste a move to guarantee the intercept
            ('14,7->17,7->17,6->18,6', -2),
            ('None', 7),
            ('14,7->14,2', 7)
        ]:
            with self.subTest(path=path):
                ...
                self.assertTileDifferentialGreaterThan(minEcon, simHost)
    ...
    def test_should_not_override_defense_when_gets_self_killed(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_override_defense_when_gets_self_killed___4blcspk3P---1--612.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 612, fill_out_tiles=True)

        rawMap, _ = self.load_map_and_general(mapFile, respect_undiscovered=True, turn=612)
    ...
    def test_should_definitely_intercept_when_nothing_good_to_do_with_army(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_definitely_intercept_when_nothing_good_to_do_with_army___ZQno-p1Xg---1--139.txtmap'
        ...
        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(enemyGeneral.player, '13,5->14,5->14,11->11,11->11,12->10,12')
        #proof
        # simHost.queue_player_moves_str(general.player, '12,14->12,11->14,11')
        bot = self.get_debug_render_bot(simHost, general.player)
        rawMap.GetTile(13, 5).delta.fromTile = rawMap.GetTile(12, 5)
        playerMap = simHost.get_player_map(general.player)
    ...
    def test_should_not_allow_runaround_to_kill_general(self):
        # TODO in theory we split backwards here, I think
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_allow_runaround_to_kill_general___4bhqHLlOh---1--335.txtmap'
    ...
    def test_should_intercept_obvious_intercept_use_case(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        for path in [
            '10,10->12,10->12,9->19,9',
            '10,10->12,10->12,8->19,8',
        ]:
            with self.subTest(path=path):
                mapFile = 'GameContinuationEntries/should_intercept_obvious_intercept_use_case___wQ-7lZL7d---0--130.txtmap'
                map, general, enemyGeneral = self.load_map_and_generals(mapFile, 139, fill_out_tiles=True)
...
# 83f 75p 4s, after VERY FIRST attempt at pruning intercept points based on distance to average tile position...? No way that stuff is even right
    ...
    def test_should_recognize_to_either_split_or_delay(self):
        ...
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        for path, proof in [
            ('3,11->4,11->4,9', '2,10->3,10z  2,10->4,10'),
            ('3,11->0,11', '2,10->3,10z  2,10->2,11'),
            ('3,11->3,9->2,9', '2,10->3,10z  2,10->3,10'),
            ('3,11->3,12->1,12', '2,10->3,10z  2,10->2,12'),
            ('3,11->2,11->2,12->1,12', '2,10->3,10z  2,10->2,11'),
        ]:
            with self.subTest(path=path):
                mapFile = 'GameContinuationEntries/should_recognize_to_either_split_or_delay____vJ88k0wu---0--233.txtmap'
                map, general, enemyGeneral = self.load_map_and_generals(mapFile, 233, fill_out_tiles=True)
                ...
                tiles = [
                    ...
                    playerMap.GetTile(2, 9),
                    playerMap.GetTile(1, 12),
                    playerMap.GetTile(1, 12),
                ]
                for tile in tiles:
                    if tile.player == enemyGeneral.player:
                        self.assertLess(tile.army, 20)

    def test_should_intercept_attack_lmao(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        # TODO this is bad. So the problem here is that we think pulling one extra army helps, dont know why though?
        for path in [
            '14,15->14,17->15,17',
            '14,15->14,17->12,17',
        ]:
...
setting bestInterceptTable[dist 1]:
  new  int 8,8@8,9->8,9: 18.68v/1t (18.68vt), re 0, dBlk 16.47, eRem -10, bct 1, wct 0, del0
    ...
    def test_should_never_intercept_away_from_threat_2(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_never_intercept_away_from_threat_2___ujwhelx13---0--133.txtmap'
        for path in ['12,7->9,7->9,3->11,3', '12,7->12,5->11,5->11,2->11,4->7,4']:
            with self.subTest(path=path):
                map, general, enemyGeneral = self.load_map_and_generals(mapFile, 133, fill_out_tiles=True)
    ...
    def test_should_not_intercept_inbound_threats_as_defense_intercepts_with_the_tile_being_threatened(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_intercept_inbound_threats_as_defense_intercepts_with_the_tile_being_threatened___x8KSpOZGG---0--353.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 353, fill_out_tiles=True)

        rawMap, _ = self.load_map_and_general(mapFile, respect_undiscovered=True, turn=353)

        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(enemyGeneral.player, '6,14->3,14->3,10->2,10')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
    ...
    def test_should_intercept_without_thinking_intercept_value_negative(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_intercept_without_thinking_intercept_value_negative___LPQCu9KIH---1--485.txtmap'
        for path in [
            '15,10->16,10->16,9->15,9->16,9->15,9->16,9',
            '15,10->16,10->16,9->14,9->14,4',
            '15,10->15,9->14,9->14,4',
        ]:
            with self.subTest(path=path):
    ...
    def test_should_always_intercept_at_the_point_adjacent_to_all_good_enemy_moves(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_always_intercept_at_the_point_adjacent_to_all_good_enemy_moves___CsETk4WLJ---0--240.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 240, fill_out_tiles=True)
        ...
        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(enemyGeneral.player, '7,7->4,7->4,3')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
        ...
        self.assertGreater(playerMap.At(6, 6).army, 20)

    def test_should_always_intercept_at_the_point_adjacent_to_all_good_enemy_moves__long(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_always_intercept_at_the_point_adjacent_to_all_good_enemy_moves___CsETk4WLJ---0--240.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 240, fill_out_tiles=True)
        ...
        self.assertLess(playerMap.At(4, 7).army, 5)
...
# 88f 131p 6s, 5 min 24s - with the claude fable 5 tweak to the something cone of allowedSearchRegion or whatever
    def test_should_not_allow_enemy_to_dodge_back_away_from_general_without_countercapping(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_allow_enemy_to_dodge_back_away_from_general_without_countercapping___xow-pf3Af---0--290.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 290, fill_out_tiles=True)
        ...
        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(enemyGeneral.player, '12,8->12,7->11,7->11,8->10,8->10,7')
        simHost.queue_player_moves_str(general.player, '14,5->13,5')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
        ...
        self.assertTileDifferentialGreaterThan(4, simHost, 'because obviously')

    def test_should_not_allow_enemy_to_dodge_back_away_from_general_without_countercapping__short(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_allow_enemy_to_dodge_back_away_from_general_without_countercapping__short___xow-pf3Af---0--294.txtmap'
        ...
        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(enemyGeneral.player, '10,8->10,7')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
