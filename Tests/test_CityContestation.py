...
class CityContestationTests(TestBase):
    ...
    def test_should_defend_forward_contested_en_city_while_capping_tiles(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.25, turns=42)
        self.assertIsNone(winner)
    ...
    def test_should_all_in_gather_to_hold_the_city(self):
        ...
        for timespan, expectedArmy in ((35, 45), (20, 20), (30, 33)):
            with self.subTest(timespan=timespan):
                ...
                self.begin_capturing_logging()
                winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.25, turns=timespan)
                self.assertIsNone(winner)
    ...
    def test_should_split_large_army_between_two_enemy_cities_rather_than_repeat_abandoning(self):
        ...
        for (name, pathAdd, numTurns) in [('defense split', '', 3), ('contest split', 'None  None  ', 5)]:
            with self.subTest(name):
                ...
                self.begin_capturing_logging()
                winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.25, turns=numTurns)
