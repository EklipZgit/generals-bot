...
class CityGatherTests(TestBase):
    ...
    def test_should_capture_nearby_neutral_city_quickly(self):
        ...
        for armyReduction in [2, 1, 0]:
            with self.subTest(armyReduction=armyReduction):
                ...
                winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.3, turns=14)
    ...
    def test_should_capture_neutral_city_quickly__messed_up_gather(self):
        ...
        # beyond 14 turns its clearly screwing up
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=1.5, turns=8)
        self.assertIsNone(winner)
        city = self.get_player_tile(12, 6, simHost.sim, map.player_index)
        self.assertEqual(general.player, city.player)
        self.assertNoRepetition(simHost)
    ...
    def test_should_capture_neutral_city_quickly__messed_up_gather__gen_1_further(self):
        ...
        # beyond 14 turns its clearly screwing up
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.1, turns=8)
        self.assertIsNone(winner)
        city = self.get_player_tile(12, 6, simHost.sim, map.player_index)
        self.assertEqual(general.player, city.player)
        self.assertNoRepetition(simHost)
    ...
    def test_should_gather_at_enemy_city__should_use_large_tiles(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.3, turns=40)
    ...
    def test_should_not_capture_enemy_cities_through_neutral_cities(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.3, turns=16)
        self.assertIsNone(winner)
    ...
    def test_should_retake_city_rapidly_and_not_throw_small_armies_at_it(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.2, turns=35)
        self.assertIsNone(winner)
    ...
    def test_should_capture_city_without_leaving_it_at_0__longer(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.5, turns=9)
        self.assertIsNone(winner)
    ...
    def test_should_capture_neutrals_as_part_of_neutral_city_capture(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.3, turns=8)
        self.assertIsNone(winner)
    ...
    def test_should_find_neutral_city_plan_in_2v2(self):
        for includeThreat in [True, False]:
            with self.subTest(includeThreat=includeThreat):
                ...
                self.begin_capturing_logging()
                winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.2, turns=16)
                self.assertNoFriendliesKilled(map, general, allyGen)
    ...
    def test_should_perform_longer_gather_to_cities(self):
        ...
        self.begin_capturing_logging()
        winner = simHost.run_sim(run_real_time=debugMode and not self.GLOBAL_BYPASS_RENDERING, turn_time=0.25, turns=40)
        self.assertIsNone(winner)
    ...
    def test_should_be_able_to_recapture_large_city_cluster(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        for keepsMoving in [True, False]:
            with self.subTest(keepsMoving=keepsMoving):
    ...
    def test_should_complete_city_gather_and_not_switch_to_offensive_city_contest_of_city_next_to_en_gen(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_complete_city_gather_and_not_switch_to_offensive_city_contest_of_city_next_to_en_gen___M85sCgnTR---0--417.txtmap'
        ...
        city = playerMap.GetTile(16,6)
        self.assertOwned(general.player, city)

    def test_should_capture_city_mas_rapido(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_capture_city_mas_rapido___zOkF05VZZ---0--213.txtmap'
        ...
        city = playerMap.GetTile(3, 6)
        self.assertOwned(general.player, city)

    def test_should_capture_city_mas_rapido__earlier(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_capture_city_mas_rapido__earlier___zOkF05VZZ---0--200.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 200, fill_out_tiles=True)
    ...
    def test_should_take_cities_deep_in_our_territory_even_though_far(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_take_cities_deep_in_our_territory_even_though_far___QiScniOcp---1--250.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 250, fill_out_tiles=True)
        ...
        cityA = playerMap.GetTile(4, 5)
        cityB = playerMap.GetTile(4, 4)
        if cityB.player != general.player and cityA.player != general.player:
            self.assertOwned(general.player, cityA)
            self.assertOwned(general.player, cityB)

    def test_should_take_far_away_neutral_when_no_other_options(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_take_far_away_neutral_when_no_other_options___ajPs0-Z_w---1--350.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 350, fill_out_tiles=True)
        ...
        city = playerMap.GetTile(10, 1)

        self.assertOwned(general.player, city)

    def test_should_not_overkill_city_and_leave_general_defenseless(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_overkill_city_and_leave_general_defenseless___XWHQOYv7I---1--468.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 468, fill_out_tiles=True)

        rawMap, _ = self.load_map_and_general(mapFile, respect_undiscovered=True, turn=468)
    ...
    def test_should_not_loop_on_choke_army_nor_randomly_waste_flank_moves__should_not_take_city_not_safe(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_loop_on_choke_army_nor_randomly_waste_flank_moves__should_not_take_city_not_safe___P7zASxjRE---1--98.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 98, fill_out_tiles=True)

        rawMap, _ = self.load_map_and_general(mapFile, respect_undiscovered=True, turn=98)

        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(enemyGeneral.player, '13,3->12,3->12,2->11,2')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
        ...
        city = playerMap.GetTile(11, 0)
        self.assertEqual(-1, city.player)

    def test_should_not_fail_to_cap_city(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_fail_to_cap_city___lG9TRizZL---0--215.txtmap'
...
# 43f, 43p, 9s
    ...
    def test_should_be_able_to_capture_city(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_be_able_to_capture_city___x6Be5D1mh---0--200.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 200, fill_out_tiles=True)
    ...
    def test_should_not_miss_move_after_completing_city_gather(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_miss_move_after_completing_city_gather___CsETk4WLJ---0--219.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 219, fill_out_tiles=True)

        rawMap, _ = self.load_map_and_general(mapFile, respect_undiscovered=True, turn=219)

        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(general.player, '6,4->6,5')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
    ...
    def test_should_not_miss_move_after_completing_city_gather__longer(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_miss_move_after_completing_city_gather___CsETk4WLJ---0--218.txtmap'
    ...
    def test_should_not_take_city_when_definitely_going_to_die_after_lol(self):
        ...
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 251, fill_out_tiles=True)

        rawMap, _ = self.load_map_and_general(mapFile, respect_undiscovered=True, turn=251)
    ...
    def test_should_complete_3_part_city_capture(self):
        ...
        self.assertGreater(bot.opponent_tracker.current_team_cycle_stats[bot._map.friendly_team].moves_spent_gathering_neutral_city_capture, 15, 'should acknowledge we spent everything we gathered this round to take the city')

    def test_should_not_decide_it_is_safe_to_take_a_city_here_overvaluing_cityDefVal(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_decide_it_is_safe_to_take_a_city_here_overvaluing_cityDefVal___h0Y9AGnQF---1--213.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 213, fill_out_tiles=True)
    ...
    def test_should_take_neutral_city_when_actively_hunting_for_one__the_fuck(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_take_neutral_city_when_actively_hunting_for_one__the_fuck___yoAbVOiKB---0--307.txtmap'
