...
class ExpansionTests(TestBase):
    ...
    def test_should_not_split_for_neutral_while_exploring_enemy_path_with_largish_army(self):
        ...
        simHost.force_lag_turn_even_with_debugger = True
...
"""
        ...
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        #proof
        # achieves 50 tile diff
        # simHost.queue_player_moves_str(general.player, '1,4->0,4->0,7  11,0->11,2  3,3->9,3->9,9')
        # achieves 51 tile diff.
        simHost.queue_player_moves_str(general.player, '1,4->1,7  11,0->11,2  3,3->9,3->9,9')
    ...
    def test_should_take_enemy_tiles_till_end_of_round_not_do_weird_neutral_crap(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_take_enemy_tiles_till_end_of_round_not_do_weird_neutral_crap___7hWF-dxy5---1--89.txtmap'
        map, general, enemyGeneral = self.load_map_and_generals(mapFile, 89, fill_out_tiles=True)
        ...
        self.enable_search_time_limits_and_disable_debug_asserts()
        simHost = GameSimulatorHost(map, player_with_viewer=general.player, playerMapVision=rawMap, allAfkExceptMapPlayer=True)
        simHost.queue_player_moves_str(general.player, '10,3->10,4')
        simHost.queue_player_moves_str(enemyGeneral.player, '6,3->6,2')
        bot = self.get_debug_render_bot(simHost, general.player)
        playerMap = simHost.get_player_map(general.player)
...
# 35f 45p with intercepts included in expansion but after fixing not using core path properly
    ...
    def test_should_not_cock_block_expansion_line_with_leaf_move_cap(self):
        debugMode = not TestBase.GLOBAL_BYPASS_REAL_TIME_TEST and True
        mapFile = 'GameContinuationEntries/should_not_intercept_starting_from_a_1_lol___vD_QzmxB8---0--432.txtmap'
