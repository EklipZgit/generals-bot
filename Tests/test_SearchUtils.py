    ...
    def test_dest_breadth_first_target__calculates_longer_even_path_values_correctly__targeting_general(self):
        ...
        for turn in turnsToTest:
            with self.subTest(turn=turn):
                ...
                sim.make_move(1, Move(map.GetTile(0, 10), map.GetTile(0, 9)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 9), map.GetTile(0, 8)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 8), map.GetTile(0, 7)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 7), map.GetTile(0, 6)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 6), map.GetTile(0, 5)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 5), map.GetTile(0, 4)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 4), map.GetTile(0, 3)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 3), map.GetTile(0, 2)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 2), map.GetTile(0, 1)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 1), map.GetTile(0, 0)))
                sim.execute_turn(dont_require_all_players_to_move=True)
    ...
    def test_dest_breadth_first_target__calculates_longer_even_path_values_correctly__targeting_general_even_path_length(self):
        ...
        for turn in turnsToTest:
            with self.subTest(turn=turn):
                ...
                sim.make_move(1, Move(map.GetTile(0, 11), map.GetTile(0, 10)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 10), map.GetTile(0, 9)))
                sim.execute_turn(dont_require_all_players_to_move=True)
                sim.make_move(1, Move(map.GetTile(0, 9), map.GetTile(0, 8)))
