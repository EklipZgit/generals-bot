...
class WatchmanRouteUtilsTests(TestBase):
    ...
    def test_a_star_watchman_route_problem_should_produce_optimal_result(self):
        ...
        toCapture = bot.get_target_player_possible_general_location_tiles_sorted(elimNearbyRange=0, player=bot.targetPlayer, cutoffEmergenceRatio=0.1)

        self.begin_capturing_logging()
        startTime = time.perf_counter()
        astar_wrp = WatchmanRouteUtils.BasicAStarWRP(startTile, playerMap, set(toCapture))
    ...
    def test_seiraf_should_find_optimal_route_on_larger_search_area(self):
        ...
        self.begin_capturing_logging()
        startTime = time.perf_counter()
        pivot_wrp = WatchmanRouteUtils.PivotWRP(startTile, playerMap, set(toCapture))
        tilePath = pivot_wrp.solve()
        completedTime = time.perf_counter() - startTime
    ...
    def test_seiraf_should_respect_cutoff_time(self):
        ...
        startTime = time.perf_counter()
        pivot_wrp = WatchmanRouteUtils.PivotWRP(startTile, playerMap, set(toCapture))
        tilePath = pivot_wrp.solve(cutoffTime=startTime + 0.05)
        completedTime = time.perf_counter() - startTime
