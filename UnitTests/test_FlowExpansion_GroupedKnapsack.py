...
class FlowExpansionGroupedKnapsackTests(TestBase):
    ...
    def test_grouped_knapsack__basic_two_island_map(self):
        """Test grouped knapsack optimization on a simple two-island map"""
...
|    |    |    |
        ...
        # Enrich capture entries with gather support (Phase 3)
        flowExpanderV2._postprocess_flow_stream_gather_capture_lookup_pairs(lookup_tables)

        # Test grouped knapsack optimization (Phase 4)
        turn_budget = 10  # Reasonable turn budget for this simple case
        solution = flowExpanderV2._solve_grouped_knapsack(lookup_tables, turn_budget)

        self.assertIsNotNone(solution, 'Should produce a solution')
        self.assertIn('selected_entries', solution, 'Solution should contain selected entries')
        self.assertIn('total_turns', solution, 'Solution should contain total turns')
        self.assertIn('total_value', solution, 'Solution should contain total value')

        # Verify solution respects turn budget
        self.assertLessEqual(solution['total_turns'], turn_budget, 'Solution should respect turn budget')

        # For this simple case, we expect at least one entry to be selected
        selected_entries = solution['selected_entries']
        self.assertGreater(len(selected_entries), 0, 'Should select at least one entry')

        # Verify selected entries are valid
        for entry in selected_entries:
            self.assertIsNotNone(entry, 'Selected entry should not be None')
            self.assertGreater(entry.turns, 0, 'Selected entry should have positive turns')
            self.assertGreater(entry.econ_value, 0, 'Selected entry should have positive value')
