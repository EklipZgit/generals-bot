                ...
                if quickKill is not None and quickKill.length > 0:
                    ...
                    if quickKill.value > cutoffValue or threatIsGeneralKill:
                        ...
                        if threat is None or not threatIsGeneralKill or threat.turns + curExtraTurn >= quickKill.length + additionalKillDist:
                            logbook.info(f"QK {quickKill.value} > {cutoffValue} in {maxDefTurns}t {quickKill.length}+{additionalKillDist} :^)")
...
Unknown message type: ['ping_tile', 125, 0]        @param pingTile:
    ...
    def get_defense_tree_move_prio_func(
        ...
        def move_closest_negative_value_func(curTile: Tile, currentPriorityObject):
            ...
            if threatDist > closenessToThreat and curTile in shortestTiles:
    ...
    def convert_int_map_matrix_to_string(self, mapMatrix: MapMatrixInterface[int]) -> str:
        return ','.join([str(MapMatrixInterface[tile]) for tile in self._map.get_all_tiles()])

    def convert_float_map_matrix_to_string(self, mapMatrix: MapMatrixInterface[float]) -> str:
        return ','.join([f'{MapMatrixInterface[tile]:.2f}' for tile in self._map.get_all_tiles()])

    def convert_bool_map_matrix_to_string(self, mapMatrix: MapMatrixInterface[bool] | MapMatrixSet) -> str:
        return ''.join(["1" if MapMatrixInterface[tile] else "0" for tile in self._map.get_all_tiles()])
