...
class EklipZBot(object):
    ...
    def perform_move_prep(self):
        ...
        if not wasFlanking and self.army_out_of_play:
            with self.perf_timer.begin_move_event('flank/oop recalculate_player_paths'):
                # TODO why was this here? the not wasFlanking means we're never changing self.flanking from true to false... all we're doing is messing with OTHER locked launch points...?
                # self.flanking = False
    ...
    def select_move(self, allowRetry=True):
                ...
                if defenseMove is not None:
                    if not self.detect_repetition(defenseMove, turns=6, numReps=3) or (threat.threatType == ThreatType.Kill and threat.path.tail.tile.isGeneral):
                        if defenseMove.source in self.largePlayerTiles and self.targetingArmy is None:
                            logging.info(f'threatDefense overriding targetingArmy with {str(threat.path.start.tile)}')
        ...
        #             logging.info(f'setting targetingArmy to {str(threat.path.start.tile)} in Kill (non-vision) threat')
        #             self.targetingArmy = self.get_army_at(threat.path.start.tile)
        ...
        if (
            ...
            if path is not None and self.worth_path_kill(path, threat.path, threat.armyAnalysis, visionKillDistance):
                if self.targetingArmy is None:
                    logging.info(f'setting targetingArmy to {str(threat.path.start.tile)} in Kill VISION threat')
                    self.targetingArmy = self.get_army_at(threat.path.start.tile)
        ...
        if haveFullExpPlanAlready or ((self.curPath is None or self.curPath.start.next is None) and not self.defend_economy or self._map.turn < 100):
            ...
            with self.perf_timer.begin_move_event("try_find_expansion_move main timing"):
                ...
                move = self.try_find_expansion_move(expNegs, timeLimit, forceBypassLaunch=haveFullExpPlanAlready)
        ...
        if self.sketchiest_potential_inbound_flank_path is not None and self.sketchiest_potential_inbound_flank_path.tail.tile in self.board_analysis.flank_danger_play_area_matrix:
            with self.perf_timer.begin_move_event('Proactive flank defense'):
                ...
                flankDefMove = self.get_flank_vision_defense_move(self.sketchiest_potential_inbound_flank_path, negs, atDist=self.board_analysis.within_flank_danger_play_area_threshold)
        ...
        if self.behavior_allow_pre_gather_greedy_leaves:
            if (
                ...
                #         return leafMove
                expMove = self.try_find_expansion_move(defenseCriticalTileSet=defenseCriticalTileSet, timeLimit=0.1, forceBypassLaunch=False, overrideTurns=self.approximate_greedy_turns_avail)
                if expMove:
    ...
    def kill_enemy_paths(self, threatPaths: typing.List[Path], allowGeneral = False) -> Path | None:
        ...
        logging.info(f"Starting kill_enemy_path for path {str(threatPath)}")
    ...
    def explore_target_player_undiscovered(self, negativeTiles) -> typing.Union[None, Path]:
        ...
        #        logging.info("explore: adding tile {} to skipTiles for lowArmy search".format(tile.toString()))
        #        skipTiles.add(tile)
    ...
    def capture_cities(
        ...
        if WinCondition.ContestEnemyCity in self.win_condition_analyzer.viable_win_conditions:
        ...
        if path:
            logging.info(f"   find_enemy_city_path returned {str(path)}")
        else:
    ...
    def find_neutral_city_path(self) -> Path | None:
        ...
        if is1v1 and wayAheadOnEcon and isNotLateGame or len(self.get_contested_targets()) > 0:
            return None
        ...
        logging.info(f'looking for neut city with thresholds relevanceCutoff {relevanceCutoff:.2f}, distRatioThreshNormal {distRatioThreshNormal:.2f}, distRatioThreshEnVisionFewCities {distRatioThreshEnVisionFewCities:.2f}, distRatioThreshEnVisionLotsOfCities {distRatioThreshEnVisionLotsOfCities:.2f}')
    ...
    def check_should_be_all_in(self) -> bool:
        ...
        if self._map.remainingPlayers - len(self.get_afk_players()) <= 2 or self._map.is_2v2:
            ...
            elif should2v2PartnerDeadAllIn or (not self.is_all_in_army_advantage and self._map.turn > 50 and enStats.tileCount + 35 * enStats.cityCount > 5 + frStats.tileCount * 1.1 + (35 * frStats.cityCount)):
                self.all_in_losing_counter += 1
            else:
                self.all_in_losing_counter = 0
            if self.all_in_losing_counter > frStats.tileCount:
                self.is_all_in_losing = True
            if enStats.tileCount + 35 * enStats.cityCount > frStats.tileCount * 1.5 + 5 + 35 * frStats.cityCount and enStats.score > frStats.score * 1.6 + 5:
                ...
                if self.giving_up_counter > frStats.tileCount + 20 and not self.finishing_exploration:
                    logging.info(
    ...
    def get_best_defense(self, defendingTile: Tile, turns: int, negativeTileList: typing.List[Tile]) -> Path | None:
        ...
        logging.info(f"Trying to get_best_defense. Turns {turns}. Searching player {searchingPlayer}")
        negativeTiles = set()
    ...
    def get_gather_to_threat_paths(
        ...
        for tile in distDict.keys():
            logging.info(f'common intercept {str(tile)} at dist {distDict[tile]}')
            ...
            for tile in largeIntercepts:
                for adj in tile.movable:
                    ...
                    if manhattan < closestManhattan or (manhattan == closestManhattan and euclid < closestXY):
                        ...
                        logging.info(f"new shortest intercept closest move {str(closestMove)}, manhattan {closestManhattan}, euclid {closestXY:.2f}")
    ...
    def get_gather_tiebreak_matrix(self) -> MapMatrix[float]:
        ...
        for tile in self._map.get_all_tiles():
            ...
            #         # logging.info(f'distMatrix out of play {str(tile)} : {outOfPlayOffset:.3f}')
    ...
    def get_defense_moves(
        ...
        for threat in threats:
            ...
            if army and army.visible and army.last_moved_turn > self._map.turn - 2:
                logging.info(f'get_defense_moves setting targetingArmy to real threat army {str(army)}')
            ...
            if threat is not None and threat.threatType == ThreatType.Kill:
                ...
                addlTurns = 0
                if isEconThreat:
                    addlTurns = max(5, self.timings.get_turns_left_in_cycle(self._map.turn) - 5)
                ...
                with self.perf_timer.begin_move_event(f'Def Gath @ {str(threat.path.start.tile)}->{str(threat.path.tail.tile)}'):
                    ...
                    if not threat.path.tail.tile.isGeneral:
                        timeLimit = 0.015
                    move, valueGathered, turnsUsed, gatherNodes = self.get_gather_to_threat_path(threat, requiredContribution=survivalThreshold, additionalNegatives=additionalNegatives, addlTurns=addlTurns, timeLimit=timeLimit)
                    ...
                    with self.perf_timer.begin_move_event(f'Def prun @ {str(threat.path.start.tile)}->{str(threat.path.tail.tile)}'):
                        if valueGathered > survivalThreshold:
                            ...
                            if threat.saveTile is not None or isEconThreat:
                                saveTurns += 1
                            leavesGreaterThanDistance = GatherUtils.get_tree_leaves_further_than_distance(pruned, threat.armyAnalysis.aMap, threat.turns - 1)
                            anyLeafIsSameDistAsThreat = len(leavesGreaterThanDistance) > 0
                            ...
                            if sumPrunedTurns >= threat.turns - 1 or anyLeafIsSameDistAsThreat:  # was - 2, but that fails this test: test_should_not_defense_loop_let_army_engine_make_moves
                                ...
                                if sumPrunedTurns >= threat.turns - 2:
                                    ...
                                    if self.territories.is_tile_in_friendly_territory(threat.path.start.tile):
                                        logging.info(f'get_defense_moves setting targetingArmy to threat in friendly territory {str(threat.path.start.tile)}')
                                        self.targetingArmy = self.get_army_at(threat.path.start.tile)
        ...
        if anyRealThreats and self.threat:
    ...
    def get_quick_kill_on_enemy_cities(self, defenseCriticalTileSet: typing.Set[Tile]) -> Path | None:
        ...
        for enemyCity in enemyCitiesOrderedByPriority:
            ...
            if self.dangerAnalyzer.fastestThreat is not None and enemyCity in self.dangerAnalyzer.fastestThreat.path.tileSet:
                ...
                logging.info(f'bypassing negativeTiles for city quick kill on {str(enemyCity)} due to it being part of threat path')
                negTilesToUse = set()
                cityDepthSearch -= 1
            ...
            if self.dangerAnalyzer.fastestPotentialThreat is not None and enemyCity in self.dangerAnalyzer.fastestPotentialThreat.path.tileSet:
                # if we got here, we either can't defend the threat or we're already safe from the threat with whats on
                # the path, and the threat includes the city, so then we're safe to use anything on the path to kill part of the threat.
                logging.info(f'bypassing negativeTiles for city quick kill on {str(enemyCity)} due to it being part of POTENTIAL threat path')
                negTilesToUse = set()
            ...
            if killPath is not None:
                ...
                if bestDef is not None and bestDef.value > killPath.value:
                    ...
                    logging.info(f'bypassed city killpath because best defense was easier for opp. {killPath.value} vs {bestDef.value}')
                    continue
        ...
        if shortestKill is not None:
            ...
            if self.dangerAnalyzer.fastestThreat is not None and tgCity in self.dangerAnalyzer.fastestThreat.path.tileSet:
                # if we got here, we either can't defend the threat or we're already safe from the threat with whats on
                # the path, and the threat includes the city, so then we're safe to use anything on the path to kill part of the threat.
                logging.info(f'bypassing negativeTiles for city quick kill on {str(tgCity)} due to it being part of threat path')
                negTilesToUse = set()
    ...
    def get_max_explorable_undiscovered_tile(self, minSpawnDist: int):
        ...
        depth = 10
        if self._map.rows * self._map.cols > 700:
            depth = 5
        elif self._map.rows * self._map.cols > 450:
            depth = 8
    ...
    def get_armies_scrim_result(
        ...
        if enemyCannotMoveAway and self.engine_allow_force_incoming_armies_towards:
            ...
            if len(enemyArmy.expectedPaths) > 0:
                ...
                logging.info(f'forcing enemy scrim moves towards {str(enemyArmy.expectedPaths[0].tail.tile)}')
            else:
    ...
    def continue_killing_target_army(self) -> Move | None:
        ...
        if self.targetingArmy.tile in self.armyTracker.armies:
            ...
            if not inExpPlan:
                ...
                return self.get_first_path_move(expPath)
    ...
    def check_for_danger_tile_moves(self) -> Move | None:
        ...
        for tile in dangerTiles:
            ...
            if self.is_move_safe_valid(move):
                ...
                logging.info(f'Setting targetingArmy to {str(tile)} in check_for_danger_tiles_move')
            ...
            if move is not None:
                ...
                if targetCity.isNeutral:
                    ...
                    prunedTurns, prunedValue, prunedGatherNodes = GatherUtils.prune_mst_to_max_army_per_turn_with_values(
                        ...
                        viewInfo=self.viewInfo if self.info_render_gather_values else None)
                # else:
                #     prunedTurns, prunedValue, prunedGatherNodes = gatherTurns, gatherValue, gatherNodes
                ...
                sameLengthKillPath = SearchUtils.dest_breadth_first_target(
                    ...
                    maxTime=0.03,
                    maxDepth=min(9, prunedTurns + len(targets) + 1),
                    noNeutralCities=True,
                    preferCapture=True,
                    negativeTiles=negativeTiles,
                    searchingPlayer=self.general.player)
                    ...
                    if self.curPath.start.tile in defenseCriticalTileSet and self.curPath.start.tile.army > 5:
                        ...
                        if threat is not None:
                            ...
                            if killThreatPath is not None:
                                ...
                                logging.info(f'setting targetingArmy to {str(threat.path.start.tile)} in continue_cur_path when move wasnt safe for general')
    ...
    def find_sketchiest_fog_flank_from_enemy(self) -> Path | None:
        ...
        # logging.info(f'Looking for flank')
        path = SearchUtils.breadth_first_dynamic_max(
            self._map,
            startTiles,
            valueFunc=valueFunc,
...
Unknown message type: ['ping_tile', 125, 0]        @param pingTile:
    ...
    def try_find_expansion_move(self, defenseCriticalTileSet: typing.Set[Tile], timeLimit: float, forceBypassLaunch: bool = False, overrideTurns: int = -1) -> Move | None:
        ...
        if path:
            ...
            if path.start.tile.isGeneral and not inLaunchSplit and len(allPaths) > 1:
                path = allPaths[1]
    ...
    def get_potential_threat_movement_negatives(self, targetTile: Tile | None = None) -> typing.Set[Tile]:
        ...
        if threatArmy is not None and not threatArmy.tile.visible:
            if potThreat.turns < 7 and self.targetingArmy is None:
                logging.info(f'get_potential_threat_movement_negatives setting targetingArmy to {str(threatArmy)} due to potential threat less than 7')
    ...
    def get_avoid_other_players_expansion_matrix(self) -> MapMatrix[float]:
        ...
        for tile in self._map.get_all_tiles():
            ...
            if self.info_render_expansion_matrix_values:
                self.viewInfo.bottomMidRightGridText[tile.x][tile.y] = f'x{matrix[tile]:0.2f}'
    ...
    def find_fog_bisection_targets(self) -> typing.Set[Tile]:
        ...
        for path in bisectPaths:
            ...
            bisects.add(tg)
            logging.info(f'BISECTS INCLUDES {str(tg)}!')
    ...
    def build_expansion_plan(
        ...
        with self.perf_timer.begin_move_event(f'optimal_expansion'):
            ...
            self.viewInfo.add_info_line(f'EXP AVAIL {expansionTurnsAvailable} en{enCaps} neut{neutCaps}')
    ...
    def build_enemy_expansion_plan(
        ...
        with self.perf_timer.begin_move_event(f'enemy optimal_expansion'):
            ...
            self.viewInfo.add_info_line(f'EN EXP AVAIL {expansionTurnsAvailable} fr{frCaps} neut{neutCaps}')
    ...
    def check_should_defend_economy_based_on_cycle_behavior(self, defenseCriticalTileSet: typing.Set[Tile]) -> bool:
        ...
        if self.enemy_attack_path is not None:
            enPath = self.enemy_attack_path.get_subsegment(halfDist + 1, end=True)

            enemyAttackPathVal = sum([t.army for t in enPath.tileList if self._map.is_tile_on_team_with(t, self.targetPlayer) and (t.visible or t.army < 8)])
            ...
            if enemyAttackPathEnOrFogTiles > halfDist + 1:
        ...
        # TODO probably related
        if not self.opponent_tracker.winning_on_economy(byRatio=1.08, offset=0 - halfDist) and not likelyInboundKillPush:
            return False
    ...
    def get_path_subsegment_to_closest_enemy_team_territory(self, path: Path) -> Path | None:
        ...
        if minIdx == 100:
            logging.info(f'No closer path to enemy territory found than the start of the path, prefer not using this path at all.')
            return None

        subsegment = path.get_subsegment(minIdx)
        logging.info(f'closest to enemy team territory was {str(subsegment.tail.tile)} at dist {minIdx}/{path.length}')
        return subsegment
    ...
    def make_second_25_move(self) -> Move | None:
        ...
        for move in prioritizedLeaves:
            ...
            for adj in src.movable:
                ...
                logging.info(f'src {str(src)} en{enDist} fr{frDist} < adj {str(adj)} en{existingAdjDist} fr{existingAdjFrDist}, skipping.')
        ...
        for tile in mustGatherTo:
            ...
            logging.info(f'setting {str(tile)} to depth {depth} / {gathTurns} (army {tile.army}, path len {path.length}, allowedAddlArmy {allowedAddlArmy})')
    ...
    def get_city_preemptive_defense_move(self, defenseCriticalTileSet: typing.Set[Tile]) -> Move | None:
        ...
        for t in targets:
            ...
            if len(tiles) < 1:
                ...
                negs.add(t)
                negs.update(self.get_n_closest_team_tiles_near([t], self.general.player, distance=max(2, self.board_analysis.inter_general_distance // 5), limit=20, includeNeutral=False))
        ...
        if move is not None:
            self.info(f'City preemptive defense {str(move)} - turns {gatherTurns}/')

        return move

    def get_flank_vision_defense_move(self, flankThreatPath: Path, negativeTiles: typing.Set[Tile], atDist: int) -> Move | None:
        included = set()
        for tile in flankThreatPath.tileList:
            for adj in tile.adjacents:
                if self.board_analysis.intergeneral_analysis.aMap[adj.x][adj.y] < atDist - 2:
                    continue
        ...
        while move is None and offset < 20:
            move, valGathered, gatherTurns, gatherNodes = self.get_gather_to_target_tiles(
                [t for t in included],
                maxTime=0.001,
                gatherTurns=offset + (50 - self._map.turn) % 5,
                maximizeArmyGatheredPerTurn=True,
                targetArmy=1,
            ...
            if valGathered / gatherTurns * 2 < playerArmyBaseline:
                logging.info(f'iterating longer on get_flank_vision_defense_move due to poor valGathered{valGathered} / gatherTurns {gatherTurns} compared to {playerArmyBaseline}')
                move = None
            offset += 5
