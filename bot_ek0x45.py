...
from base.client.map import Player, new_tile_grid, Tile, MapBase, PLAYER_CHAR_INDEX_PAIRS, PLAYER_CHAR_BY_INDEX, new_value_grid
...
class ExpansionPotential(object):
    def __init__(self, turnsUsed: int, enTilesCaptured: int, neutTilesCaptured: int, path: Path | None, allPaths: typing.List[Path], scores: typing.Dict[Path, int]):
...
class EklipZBot(object):
    def __init__(self):
        ...
        self.info_render_board_analysis_choke_widths: bool = True
    ...
    def init_turn(self, secondAttempt=False):
        ...
        if self.general is not None:
            self._gen_distances = SearchUtils.build_distance_map_incl_mountains(self._map, [self.general])
        ...
        if self.board_analysis.central_defense_point and self.board_analysis.intergeneral_analysis:
            ...
            if (self.locked_launch_point is None or self.locked_launch_point == self.general) and self.board_analysis.intergeneral_analysis.bMap[centralPoint.x][centralPoint.y] < self.board_analysis.inter_general_distance:
            ...
            for team in self.teams:
                ...
                if annihilatedFog > 0:
                    for player in self._map.players:
                        if player.team == team:
                            self.armyTracker.try_find_army_sink(player.index, annihilatedFog, tookNeutCity=self.did_player_just_take_fog_city(player.index))
    ...
    def get_timings(self) -> Timings:
        ...
        if self.target_player_gather_path is not None and self.target_player_gather_path.start.tile == self.teammate_general:
    ...
    def select_move(self, is_lag_move=False):
        ...
        if self._map.turn < 100 and not self.is_ffa_situation() and not self.completed_first_100 and not self._map.is_2v2:
            second = self.make_second_25_move()
            if second:
                return second

        # TODO this prevents interception expansion :(
        # if self.dangerAnalyzer.fastestPotentialThreat is not None:
        #     defenseCriticalTileSet.update([t for t in self.dangerAnalyzer.fastestPotentialThreat.path.tileList])
        ...
        with self.perf_timer.begin_move_event('try_get_enemy_territory_exploration_continuation_move'):
            largeArmyExpContinuationMove = self.try_get_enemy_territory_exploration_continuation_move(defenseCriticalTileSet)
        ...
        if self.sketchiest_potential_inbound_flank_path is not None and self.sketchiest_potential_inbound_flank_path.tail.tile in self.board_analysis.flank_danger_play_area_matrix:
            with self.perf_timer.begin_move_event('Proactive flank defense'):
                negs = set(defenseCriticalTileSet)
                negs.update(self.win_condition_analyzer.defend_cities)
                negs.update(self.win_condition_analyzer.target_cities)
                negs.update([p.start.tile for p in self.expansion_plan.all_paths if p.start.tile.delta.armyDelta == 0])
                flankDefMove = self.get_flank_vision_defense_move(
                ...
                negs.update(self.win_condition_analyzer.defend_cities)
                negs.update(self.win_condition_analyzer.target_cities)
                # negs.update([p.start.tile for p in self.expansion_plan.all_paths])
                flankDefMove = self.get_flank_vision_defense_move(
                    self.sketchiest_potential_inbound_flank_path,
                    negs,
                    atDist=self.board_analysis.within_flank_danger_play_area_threshold)
        ...
        if self.behavior_allow_pre_gather_greedy_leaves:
            if (
                    (self._map.remainingPlayers == 2 or self._map.is_2v2)
                    # and not self.is_player_spawn_cramped()
                    and not self.opponent_tracker.winning_on_economy(byRatio=1.35, cityValue=4, offset=self.behavior_pre_gather_greedy_leaves_offset)
                ...
                if not self.timings.in_launch_timing(self._map.turn):
                    negatives.update(self.target_player_gather_path.tileList)
                # if not self.is_all_in():
                ...
                #         return leafMove
                expMove = self.try_find_expansion_move(defenseCriticalTileSet=negatives, timeLimit=0.1, forceBypassLaunch=False, overrideTurns=self.approximate_greedy_turns_avail)
                if expMove:
                    self.info(f'greedy exp move {str(expMove)}')
                    return expMove
    ...
    def kill_army(
        ...
        if len(army.expectedPaths) == 0:
            army.expectedPaths = self.armyTracker.get_army_expected_path(army)
    ...
    def get_tree_move_default(
        ...
        if priorityFunc is None:
            ...
            #     logbook.info("Using default_high_cities_func for gather prio. player.cityCount {} > 4 or nonCityLeafCount {} < 4".format(player.cityCount, nonCityLeafCount))
        ...
        return GatherUtils.get_tree_move(gathers, priorityFunc, valueFunc, pop=pop)
    ...
    def get_threat_killer_move(self, threat: ThreatObj, searchTurns, negativeTiles) -> Move | None:
        ...
        if threat.turns > self.shortest_path_to_target_player.length // 2 and self.board_analysis.intergeneral_analysis.bMap[threatTile.x][threatTile.y] < threat.turns > self.shortest_path_to_target_player.length // 2:
    ...
    def find_neutral_city_path(self) -> Path | None:
        ...
        logbook.info(f'looking for neut city with thresholds relevanceCutoff {relevanceCutoff:.2f}, distRatioThreshNormal {distRatioThreshNormal:.2f}, distRatioThreshEnVisionFewCities {distRatioThreshEnVisionFewCities:.2f}, distRatioThreshEnVisionLotsOfCities {distRatioThreshEnVisionLotsOfCities:.2f}')
    ...
    def prioritize_expansion_leaves(
            ...
            allLeaves=None,
            allowNonKill=False,
            distPriorityMap: typing.List[typing.List[int]] | None = None,
    ) -> typing.List[Move]:
        queue = PriorityQueue()
        ...
        for leafMove in allLeaves:
            ...
            distEnemyPoints = (analysis.aMap[dest.x][dest.y] + 1) / (distPriorityMap[dest.x][dest.y] + 1)

            points += distEnemyPoints / 3

            logbook.info("leafMove {}, points {:.2f} (distEnemyPoints {:.2f})".format(leafMove.toString(), points, distEnemyPoints))
            ...
            if targetPlayerObj is None or not targetPlayerObj.knowsKingLocation:
                for genLaunchPoint in self.launchPoints:
                    if genLaunchPoint is None:
                        ...
                        logbook.info(f"using launchPoint {genLaunchPoint.toString()}")
        ...
        for x in range(self._map.cols):
            for y in range(self._map.rows):
                ...
                if tile.player == self._map.player_index:
                    ...
                    if tile.player == self.targetPlayer:
                        for nextTile in tile.movable:
                            if not self._map.is_tile_on_team_with(nextTile, self.targetPlayer) and not nextTile.isObstacle:
    ...
    def get_predicted_target_player_general_location(self, skipDiscoveredAsNeutralFilter: bool = False) -> Tile:
        ...
        for tile in self.pathableTiles:
            ...
            if self.armyTracker.emergenceLocationMap[self.targetPlayer][tile.x][tile.y] > 0:
                # foundValue += emergenceLogFactor * math.log(self.armyTracker.emergenceLocationMap[self.player][tile.x][tile.y], 2)
    ...
    def dump_turn_data_to_string(self):
        ...
        for player in self._map.players:
            ...
            data.append(f'{char}Emergences={self.convert_float_tile_2d_array_to_string(self.armyTracker.emergenceLocationMap[player.index])}')
    ...
    def load_resume_data(self, resume_data: typing.Dict[str, str]):
        ...
        for player in self._map.players:
            ...
            if f'{char}Emergences' in resume_data:
                self.armyTracker.emergenceLocationMap[player.index] = self.convert_string_to_float_tile_2d_array(resume_data[f'{char}Emergences'])
                ...
                self.armyTracker.emergenceLocationMap[self.targetPlayer][self.targetPlayerExpectedGeneralLocation.x][self.targetPlayerExpectedGeneralLocation.y] = 5
    ...
    def get_gather_to_threat_paths(
        ...
        # gatherDepth = max([threats[0].armyAnalysis.bMap[t.x][t.y] for t in commonInterceptPoints.keys()]) - 1 + addlTurns
        gatherDepth = threats[0].path.length - 1 + addlTurns
        # #
        # # commonInterceptPoints = {}
        # # largeIntercepts = []
        # # middlest = threats[0].armyAnalysis.tileA
        # # middlestDist = threats[0].turns + 1
        # # for threat in threats:
        # #     largeTiles = self.find_large_tiles_near(list(threat.armyAnalysis.shortestPathWay.tiles), threat.turns // 3 + 1, forPlayer=self.general.player, minArmy=max(threat.threatValue, threat.path.start.tile.army - 5), addlFilterFunc=lambda t, d: threat.armyAnalysis.bMap[t.x][t.y] <= threat.armyAnalysis.shortestPathWay.distance)
        # #     largeIntercepts.extend(largeTiles)
        # #     for choke in threat.armyAnalysis.pathChokes:
        # #         if choke not in threat.armyAnalysis.shortestPathWay.tiles:
        # #             continue
        # #         containedInAll = True
        # #         sumDist = threat.armyAnalysis.pathWayLookupMatrix[choke].distance
        # #         for otherThreat in threats:
        # #             if otherThreat == threat:
        # #                 continue
        # #             if choke not in otherThreat.armyAnalysis.pathChokes or choke not in otherThreat.armyAnalysis.shortestPathWay.tiles:
        # #                 containedInAll = False
        # #                 break
        # #
        # #             sumDist += otherThreat.armyAnalysis.pathWayLookupMatrix[choke].distance
        # #
        # #         if containedInAll:
        # #             commonInterceptPoints[choke] = sumDist
        # #             middleDist = abs(threat.armyAnalysis.bMap[choke.x][choke.y] - threat.turns // 2)
        # #             if middleDist < middlestDist:
        # #                 middlest = choke
        # #                 middlestDist = middleDist
        # #
        # # if interceptArmy and len(largeIntercepts) > 0:
        # #     self.viewInfo.add_info_line(f'large intercept attempt')
        # #     closestXY = 100.0
        # #     closestManhattan = 100
        # #     closestMove = None
        # #     for tile in largeIntercepts:
        # #         for adj in tile.movable:
        # #             manhattan = threats[0].armyAnalysis.bMap[adj.x][adj.y]
        # #             euclid = self.euclidDist(adj.x, adj.y, middlest.x, middlest.y)
        # #             if manhattan < closestManhattan or (manhattan == closestManhattan and euclid < closestXY):
        # #                 closestMove = Move(tile, adj)
        # #                 closestXY = euclid
        # #                 closestManhattan = manhattan
        # #                 logbook.info(f"new shortest intercept closest move {str(closestMove)}, manhattan {closestManhattan}, euclid {closestXY:.2f}")
        # #
        # #     if closestMove is not None:
        # #         self.info(f"awwwwwww shit intercept closest move {str(closestMove)}, manhattan {closestManhattan}, euclid {closestXY:.2f}")
        # #         return closestMove, closestMove.source.army, closestManhattan, []
        # #
        # # intercepts = {}
        # # # tileCutoff = max(threats[0].threatValue // 4, threats)
        # # for tile, width in threats[0].armyAnalysis.interceptChokes.items():
        # #     if self.info_render_intercept_data:
        # #         self.viewInfo.bottomRightGridText[tile.x][tile.y] = f'iw{width}'
        # #     # todo change to shortest path set
        # #     if tile not in threats[0].path.tileSet: # and tile.player == self.general.player and tile.army < threats[0].
        # #         continue
        # #     aDist = threats[0].armyAnalysis.aMap[tile.x][tile.y]
        # #     bDist = threats[0].armyAnalysis.bMap[tile.x][tile.y]
        # #     totalDist = threats[0].armyAnalysis.shortestPathWay.distance
        # #     # if bDist > totalDist // 4:  # and aDist > totalDist // 4
        # #     if aDist > totalDist // 4:  # and aDist > totalDist // 4
        # #         iDist = bDist * 2 + width - 1
        # #         intercepts[tile] = iDist
        # #         if self.info_render_intercept_data:
        # #             self.viewInfo.bottomLeftGridText[tile.x][tile.y] = f'i{iDist}'
        # #
        # #             self.viewInfo.add_targeted_tile(tile, TargetStyle.WHITE, radiusReduction=4)
        #
        # skipTiles = set()
        # if additionalNegatives is not None:
        #     skipTiles = additionalNegatives.copy()
        # for tile in self._map.get_all_tiles():
        #     aDist = threats[0].armyAnalysis.aMap[tile.x][tile.y]
        #     if aDist > threats[0].path.length:
        #         skipTiles.add(tile)
        #
        # with self.perf_timer.begin_move_event(f'intercept gath @{str(threats[0].path.start.tile)}'):
        #     move, value, turnsUsed, gatherNodes = self.try_threat_gather(
        #         threats=threats,
        #         distDict=intercepts,
        #         gatherDepth=gatherDepth * 2 + 5,
        #         force_turns_up_threat_path=force_turns_up_threat_path,
        #         requiredContribution=requiredContribution,
        #         gatherMax=False,
        #         additionalNegatives=skipTiles,
        #         timeLimit=timeLimit,
        #         pruneDepth=gatherDepth,
        #         shouldLog=shouldLog,
        #         fastMode=True)
        #
        # if move is None or value < requiredContribution:
        #     self.info(f' int gath {value:.2f}/{requiredContribution} ({turnsUsed}t) {str(move)}  for {str(threats[0].path.start.tile)}@{str(threats[0].path.tail.tile)}')
        distDict = threats[0].convert_to_dist_dict(allowNonChoke=force_turns_up_threat_path != 0)

        with self.perf_timer.begin_move_event(f'real gath @{str(threats[0].path.start.tile)}'):
            move, value, turnsUsed, gatherNodes = self.try_threat_gather(
                threats=threats,
                distDict=distDict,
                ...
                shouldLog=shouldLog)
        # else:
        #     self.info(f'USING INT GATH {value:.2f}/{requiredContribution} ({turnsUsed}t) {str(move)}  for {str(threats[0].path.start.tile)}@{str(threats[0].path.tail.tile)}')
    ...
    def try_threat_gather(
        ...
        if force_turns_up_threat_path == 0:
            move_closest_priority_func, move_closest_value_func = self.get_defense_tree_move_prio_funcs(threats[0])
        ...
        for tile in self._map.pathableTiles:
            prioMatrix[tile] = 0.0001 * threats[0].armyAnalysis.aMap[tile.x][tile.y]  # reward distances further from the threats target, pushing us to intercept further up the path. In theory?
    ...
    def check_for_king_kills_and_races(self, threat: ThreatObj, force: bool=False) -> typing.Tuple[typing.Union[Move, None], typing.Union[Path, None], bool]:
        ...
        for enemyGeneral in self.largeTilesNearEnemyKings.keys():
            ...
            if not enemyGeneral.isGeneral and not self.is_ffa_situation():
            ...
            altEnGenPositions = self.get_target_player_possible_general_location_tiles_sorted(elimNearbyRange=4, player=enemyGeneral.player)
                ...
                if killPath is not None and killPath.length > 0:
                    ...
                    if threat is None or threat.threatType != ThreatType.Kill or threatDistCutoff >= killPath.length:
                        ...
                        if kingKillPath is None:
                            ...
                            if threatDistCutoff + 1 == killPath.length:
                                logbook.info("       CAN RACE THOUGH!")
                                canRace = True
            ...
            if (killPath is not None and killPath.length >= 0) and (threat is None or threat.threatType != ThreatType.Kill or (threatDistCutoff >= killPath.length)):
                ...
                if kingKillPath is None:
                    logbook.info("  saving above kingKillPath as backup in case we can't defend threat")
                    if threatDistCutoff + 1 == killPath.length:
                        self.viewInfo.add_info_line("     CAN RACE THOUGH!")
                        canRace = True
            ...
            if self.is_ffa_situation():
                ...
                if len(tiles) > 0:
                    ...
                    if (path is not None and path.length >= 0) and (threat is None or threat.threatType != ThreatType.Kill or ((threatDistCutoff >= path.length or self.is_all_in()) and threat.threatPlayer == king.player)):
                        ...
                        if kingKillPath is None:
                            logbook.info("    saving above kingKillPath as backup in case we can't defend threat")
                            if threatDistCutoff + 1 == path.length:
                                self.viewInfo.add_info_line("     CAN RACE THOUGH!")
                                canRace = True
                            kingKillPath = path
    ...
    def get_gather_tiebreak_matrix(self) -> MapMatrix[float]:
        ...
        leaves = set()
        for i, leafMove in enumerate(prioritized):
            if i < 10:
                matrix[leafMove] += -5.0 / (i + 10)
            leaves.add(leafMove)
        ...
        for tile in self._map.get_all_tiles():
            ...
            if self.info_render_gather_values and self._map.is_tile_friendly(tile) and matrix[tile] != 0:
                self.viewInfo.bottomLeftGridText[tile.x][tile.y] = f'gm{matrix[tile]:.3f}'
    ...
    def get_defense_moves(
        ...
        for threat in threats:
            ...
            if threatTile.delta.fromTile:
                threatDist = threat.armyAnalysis.aMap[threatTile.x][threatTile.y]
                threatFromDist = threat.armyAnalysis.aMap[threatTile.delta.fromTile.x][threatTile.delta.fromTile.y]
            ...
            if threat is not None and threat.threatType == ThreatType.Kill:
                ...
                #         and army.last_moved_turn > self._map.turn - 2
                #         and not threatMovingWrongWay
                #         and not self.detect_repetition_at_all()
                # ):
                #     furthestForwardChoke = max(threat.armyAnalysis.interceptChokes.keys(), key=lambda t: threat.armyAnalysis.aMap[t.x][t.y])
                #     furthestDist = threat.armyAnalysis.aMap[furthestForwardChoke.x][furthestForwardChoke.y]
                #     distOffset = furthestDist
                ...
                with self.perf_timer.begin_move_event(f'Def Gath @ {str(threat.path.start.tile)}->{str(threat.path.tail.tile)}'):
                    ...
                    if gatherNodes is not None:
                        leavesGreaterThanDistance = GatherUtils.get_tree_leaves_further_than_distance(gatherNodes, threat.armyAnalysis.aMap, threat.turns - distOffset)
                        ...
                        move = self.get_tree_move_default(gatherNodes, move_closest_priority_func, move_closest_value_func)
                if move:
                    with self.perf_timer.begin_move_event(f'Def prun @ {str(threat.path.start.tile)}->{str(threat.path.tail.tile)}'):
                        if valueGathered > survivalThreshold:
                            ...
                            if threat.saveTile is not None or isEconThreat:
                                saveTurns += 1
                            leavesGreaterThanDistance = GatherUtils.get_tree_leaves_further_than_distance(pruned, threat.armyAnalysis.aMap, threat.turns - distOffset, survivalThreshold, sumPruned)
                            anyLeafIsSameDistAsThreat = len(leavesGreaterThanDistance) > 0
                            if anyLeafIsSameDistAsThreat:
                                flags = f'leafDist {flags}'
                            else:
                                leavesGreaterThanBlockDistance = GatherUtils.get_tree_leaves_further_than_distance(pruned, threat.armyAnalysis.aMap, saveTurns - distOffset)
                    ...
                    with self.perf_timer.begin_move_event(f'+{extraTurns} Defense Threat Gather'):
                        ...
                        if pruneToValuePerTurn:
                            sumPrunedTurns, sumPruned, altGatherNodes = GatherUtils.prune_mst_to_army_with_values(
                                ...
                                preferPrune=self.expansion_plan.preferred_tiles if self.expansion_plan is not None else None,
                                noLog=False)
                            prioFunc, valFunc = self.get_defense_tree_move_prio_funcs(threat, anyLeafIsSameDistAsThreat=False)
                            altMove = self.get_tree_move_default(altGatherNodes, prioFunc, valFunc)
            ...
            if legacySavePath is not None and legacySavePath.length > 0:
                legacySavePath.calculate_value(self.general.player, teams=self._map._teams)
                logbook.info(
    ...
    def get_quick_kill_on_enemy_cities(self, defenseCriticalTileSet: typing.Set[Tile]) -> Path | None:
        ...
        for enemyCity in enemyCitiesOrderedByPriority:
            ...
            if killPath is not None:
                bestDef = self.get_best_defense(killPath.tail.tile, killPath.length, list())
    ...
    def prep_view_info_for_render(self, move: Move | None):
        ...
        if self.info_render_centrality_distances:
            for tile in self._map.get_all_tiles():
                self.viewInfo.bottomLeftGridText[tile.x][tile.y] = f'c{self.board_analysis.defense_centrality_sums[tile]}'
        ...
        if self.info_render_board_analysis_choke_widths and self.board_analysis.intergeneral_analysis:
            for tile in self._map.get_all_tiles():
                ...
                self.viewInfo.topRightGridText[tile.x][tile.y] = f'cw{w}'
    ...
    def get_armies_scrim_result(
        ...
        if self.engine_include_path_pre_expansion:
            ...
            for enArmy in enemyArmies:
                altPaths = self.armyTracker.get_army_expected_path(enArmy)
    ...
    def build_intercept_plans(self) -> typing.Dict[Tile, ArmyInterception]:
                ...
                for otherTile, otherThreats in threatsByTile.items():
                    ...
                    for threat in otherThreats:
                        for chokeTile in threat.path.tileList:
                            ...
                            existingBlock = blockingTiles.get(chokeTile, 0)
                            if existingBlock < threat.threatValue:
                                blockingTiles[chokeTile] = threat.threatValue
                                break

                self.viewInfo.add_info_line(f'for threat {str(tile)}, blocking tiles were {str([f"{str(t)}:{v}" for t, v in blockingTiles.items()])}')
        ...
        # # find interceptions that need to split
        # for tile, interception in interceptions.items():
        #     threats = threatsByTile[tile]
        #
        #     blockingTiles = {}
        #
        #     for otherTile, otherIntercept in interceptions.items():
        #         if otherTile == tile:
        #             continue
        #         otherThreats = threatsByTile[otherTile]
        #         # for chokeTile in plan.common_intercept_choke_widths.keys():
        #         #     if not self._map.is_tile_friendly(chokeTile):
        #         #         continue
        #         #     if chokeTile.army < tile.army // 3:
        ...
        #                 blockingTiles[chokeTile] = threat.threatValue
        #
        #     self.viewInfo.add_info_line(f'for threat {str(tile)}, blocking tiles were {str([t for t in blockingTiles])}')
    ...
    def get_optimal_first_25_plan_move(self, timeLimit: float = 4.0) -> Move | None:
        calcedThisTurn = False
        distMap, skipTiles = self.get_initial_expansion_distance_priority_map()
        ...
        if self._map.turn > 50:
            normalExpansion = self.get_standard_expansion_capture_weight_matrix()
            for tile in self._map.get_all_tiles():
                distMap[tile.x][tile.y] = normalExpansion[tile]

        pruneCutoff = 14
        if self._map.turn > 25:
            pruneCutoff = 0
        ...
        if self.city_expand_plan is None or len(self.city_expand_plan.plan_paths) == 0:
            with self.perf_timer.begin_move_event('optimize_first_25'):
                ...
                while len(self.city_expand_plan.plan_paths) > 0 and self.city_expand_plan.plan_paths[0] is None:
        ...
        if self.city_expand_plan.launch_turn < self._map.turn and not calcedThisTurn and not SearchUtils.any_where(self.player.tiles, lambda tile: not tile.isGeneral and SearchUtils.any_where(tile.movable, lambda mv: not mv.isObstacle and tile.army - 1 > mv.army and not self._map.is_tile_friendly(mv))):
            ...
            distToGenMap = SearchUtils.build_distance_map(self._map, self.get_target_player_possible_general_location_tiles_sorted(elimNearbyRange=7)[0:3])

            cutoff = time.perf_counter() + 0.25
            optionalNewExpandPlan = EarlyExpandUtils.optimize_first_25(self._map, source, distMap, skipTiles=skipTiles, cutoff_time=cutoff, prune_cutoff=self.city_expand_plan.tile_captures)
    ...
    def try_find_gather_move(
        ...
        sumEnTerrArmy = 0
        for tile in genPlayer.tiles:
            # if self._map.is_player_on_team_with(self.territories.territoryMap[tile.x][tile.y], self.player) and tile.army < 8:
        ...
        if len(inEnTerrSet) < self.player.tileCount // 6 or self.opponent_tracker.get_current_team_scores_by_player(self.player.index).standingArmy - sumEnTerrArmy < self.opponent_tracker.get_current_team_scores_by_player(self.targetPlayer).standingArmy * 0.9:
    ...
    def find_sketchiest_fog_flank_from_enemy(self) -> Path | None:
        ...
        def valueFunc(tile: Tile, prioVals) -> typing.Tuple | None:
            ...
            return 0 - self.board_analysis.intergeneral_analysis.aMap[tile.x][tile.y], dist
        ...
        def prioFunc(tile: Tile, prioVals) -> typing.Tuple | None:
            ...
            return dist + 1, self.board_analysis.intergeneral_analysis.aMap[tile.x][tile.y]
            ...
            if oppStats is not None:
                ...
                if cycleTurnsLeft > self.target_player_gather_path.length and fogRisk > pathWorth and self._map.turn > 80:
                    ...
                    if self.locked_launch_point is not None and self.locked_launch_point != self.general:
                        self.viewInfo.add_info_line(f're-centering gen launch point due to {fogRisk} fog risk > our path {pathWorth}, risk in {pushRiskTurns}')
                        self.locked_launch_point = self.general
                        self.recalculate_player_paths(force=True)

                    return None

                self.viewInfo.add_info_line(f'NOT bypass launch, fogRisk {fogRisk} in {pushRiskTurns} (gath {oppStats.approximate_army_gathered_this_cycle}) vs {pathWorth} - {cycleTurnsLeft} vs len {self.target_player_gather_path.length}')
...
Unknown message type: ['ping_tile', 125, 0]        @param pingTile:
    ...
    def get_defense_tree_move_prio_funcs(
        ...
        def move_closest_priority_func(nextTile: Tile, currentPriorityObject):
            ...
            if currentPriorityObject:
                (_, _, _, lastTile) = currentPriorityObject
            ...
            return (
                ...
                # lastTile is None or (lastTile not in threat.armyAnalysis.shortestPathWay.tiles and closenessToThreat >= -1),
        ...
        def move_closest_value_func(curTile: Tile, currentPriorityObject):
            ...
            closenessToThreat = 0 - threatenedTileDistMap[curTile.x][curTile.y]
            return (
                not (anyLeafIsSameDistAsThreat or curTile.isCity),
                # lastTile in threat.armyAnalysis.shortestPathWay.tiles or closenessToThreat >= -1,
                0 - closenessToThreat,
                curTile.army
            )
    ...
    def get_capture_first_tree_move_prio_funcs(
            self
    ) -> typing.Tuple[typing.Callable[[Tile, typing.Any], typing.Any], typing.Callable[[Tile, typing.Any], typing.Any]]:
        def capture_first_priority_func(nextTile: Tile, currentPriorityObject):
            lastTile = None
            if currentPriorityObject:
                (_, lastTile) = currentPriorityObject
            return (
                lastTile is not None and lastTile.player == self.general.player,
                nextTile
            )
        ...
        return capture_first_priority_func, capture_first_value_func
    ...
    def hunt_for_fog_neutral_city(self, negativeTiles: typing.Set[Tile]) -> typing.Tuple[Path | None, Move | None]:
        ...
        SearchUtils.breadth_first_foreach_dist(
            ...
            skipFunc=lambda t: self.territories.is_tile_in_enemy_territory(t) or t.isUndiscoveredObstacle or (t.isCity and not self._map.is_tile_friendly(t)),
        ...
        for tile in self._map.get_all_tiles():
            self.viewInfo.midRightGridText[tile.x][tile.y] = f'fa{fogObstacleAdjacents[tile]}'
    ...
    def try_find_main_timing_expansion_move_if_applicable(self, defenseCriticalTileSet: typing.Set[Tile]) -> Move | None:
        ...
        if (
                self.expansion_plan.turns_used >= self.timings.get_turns_left_in_cycle(self._map.turn) - self.player.cityCount * 2
                and self.expansion_plan.en_tiles_captured > self.expansion_plan.neut_tiles_captured // 2
        ...
        if haveFullExpPlanAlready or havePotentialIntercept or ((self.curPath is None or self.curPath.start.next is None) and not self.defend_economy or self._map.turn < 100):
            ...
            if move is not None:
                if not self.timings.in_expand_split(self._map.turn):
                    cycleTurn = self.timings.get_turn_in_cycle(self._map.turn)
    ...
    def try_find_expansion_move(self, defenseCriticalTileSet: typing.Set[Tile], timeLimit: float, forceBypassLaunch: bool = False, overrideTurns: int = -1) -> Move | None:
        ...
        if (
            ...
            with self.perf_timer.begin_move_event(f'EXP - first25 reuse - {remainingTime:.3f}'):
                move = self.get_optimal_first_25_plan_move(timeLimit=remainingTime)
    ...
    def get_target_player_possible_general_location_tiles_sorted(
        ...
        if player == self.targetPlayer:
            emergenceVal = self.armyTracker.emergenceLocationMap[player][self.targetPlayerExpectedGeneralLocation.x][self.targetPlayerExpectedGeneralLocation.y]
        ...
        if player == self.targetPlayer and self.targetPlayerExpectedGeneralLocation is not None:
    ...
    def get_initial_expansion_distance_priority_map(self) -> typing.Tuple[typing.List[typing.List[int]], typing.Set[Tile]]:
        """
        ...
        tgs = self.get_target_player_possible_general_location_tiles_sorted(elimNearbyRange=10)[0:3]

        if len(tgs) < 3:
            tgs = self.get_target_player_possible_general_location_tiles_sorted(elimNearbyRange=7)[0:3]

        for tg in tgs:
            self.viewInfo.add_targeted_tile(tg, TargetStyle.TEAL)

        enDistMap = SearchUtils.build_distance_map(self._map, tgs)
        ...
        if self._map.is_2v2 and self.teammate_general is not None:
            ...
            usDist = SearchUtils.build_distance_map(self._map, [self.general])

            for tile in self._map.get_all_tiles():
                distMap[tile.x][tile.y] = 0 - distMap[tile.x][tile.y]
                distMap[tile.x][tile.y] += enDistMap[tile.x][tile.y]  # - enDistMap[self.general.x][self.general.y]

            teammateDistanceDropoffPoint = 19  # the tiles after which we stop considering distance from teammate relevant
            for teammate in self._map.teammates:
                teammateDistances = SearchUtils.build_distance_map(self._map, [self._map.generals[teammate]])
                for x in range(self._map.cols):
                    for y in range(self._map.rows):
                        # distMap[x][y] -= teammateDistances[x][y]
                        if teammateDistances[x][y] < usDist[x][y]:
                            distMap[x][y] += 3 * teammateDistanceDropoffPoint - 3 * teammateDistances[x][y]
                        if teammateDistances[x][y] < 3 and usDist[x][y] + 1 > teammateDistances[x][y]:
                            skipTiles.add(self._map.GetTile(x, y))
        elif self._map.remainingPlayers == 2 or self._map.is_2v2:
            distSource.append(self.general)
            distMap = SearchUtils.build_distance_map(self._map, distSource)
            for tile in self._map.get_all_tiles():
                distMap[tile.x][tile.y] = 0 - distMap[tile.x][tile.y]
                distMap[tile.x][tile.y] += enDistMap[tile.x][tile.y]
                distMap[tile.x][tile.y] += self.get_distance_from_board_center(tile, center_ratio=0.85)
            ...
            for tile in self._map.get_all_tiles():
                ...
                # distMap[tile.x][tile.y] = 0 - distMap[tile.x][tile.y]
                distMap[tile.x][tile.y] -= self.get_distance_from_board_center(tile, center_ratio=0.15)
        else:
            raise AssertionError("The fuck?")

        for tile in self._map.get_all_tiles():
            if isinstance(distMap[tile.x][tile.y], float):
                self.viewInfo.midLeftGridText[tile.x][tile.y] = f'e{distMap[tile.x][tile.y]:.1f}'
            else:
                self.viewInfo.midLeftGridText[tile.x][tile.y] = f'e{str(distMap[tile.x][tile.y])}'
    ...
    def get_avoid_other_players_expansion_matrix(self) -> MapMatrix[float]:
        matrix = MapMatrix(self._map, 0.0)
        for tile in self._map.get_all_tiles():
            if self.targetPlayer != -1 and (tile.player == self.targetPlayer or self.territories.territoryMap[tile.x][tile.y] == self.targetPlayer):
                if tile in self.board_analysis.intergeneral_analysis.shortestPathWay.tiles:
                    continue
            ...
            for adj in tile.adjacents:
                if not adj.discovered and not adj.isObstacle:
                    matrix[tile] -= 0.1
                if self.targetPlayer != -1 and (adj.player == self.targetPlayer or self.territories.territoryMap[adj.x][adj.y] == self.targetPlayer):
                    matrix[tile] = 0.0
                    break
            if self.info_render_expansion_matrix_values:
                self.viewInfo.bottomMidRightGridText[tile.x][tile.y] = f'x{matrix[tile]:0.3f}'
    ...
    def get_standard_expansion_capture_weight_matrix(self) -> MapMatrix[float]:
        ...
        if self.sketchiest_potential_inbound_flank_path is not None:
            for tile in self.sketchiest_potential_inbound_flank_path.tileList:
                if not tile.visible:
                    matrix[tile] += 0.15
                    for adj in tile.adjacents:
                        matrix[adj] += 0.05
        ...
        for tile in self._map.get_all_tiles():
            ...
            if pathway is not None:
                ...
                if coreDist > 0:
                    ...
                    if not tile.visible and tile.isNeutral and self._map.is_player_on_team_with(self.targetPlayer, self.territories.territoryMap[tile.x][tile.y]):
                        isEnTile = True
                    if isEnTile:
                        factor = 1.0
            ...
            excessDist = self.board_analysis.intergeneral_analysis.bMap[tile.x][tile.y] - self.board_analysis.inter_general_distance
            if excessDist > 0:
        ...
        for tile in self._map.get_all_tiles():
            if self._map.is_tile_friendly(tile):
                ...
                self.viewInfo.bottomLeftGridText[tile.x][tile.y] = f'x{matrix[tile]:0.3f}'
        return matrix
    ...
    def find_fog_bisection_targets(self) -> typing.Set[Tile]:
        ...
        for tile in self._map.get_all_tiles():
            self.viewInfo.bottomMidLeftGridText[tile.x][tile.y] = f'{"b" if candidates[tile] else "n"}{distances[tile]}'
                ...
                for t in otherPath.tileList:
                    ...
                    if planOpt is not None:
                        ...
                        for turns, (econValue, p) in planOpt.intercept_options.items():
                            if p.start.tile == otherPath.start.tile:
    ...
    def check_launch_against_expansion_plan(self, existingPlan: ExpansionPotential, expansionNegatives: typing.Set[Tile]) -> ExpansionPotential:
        if self.target_player_gather_path is None or self.target_player_gather_path.start.tile in expansionNegatives:
            return existingPlan

        turnsLeftInCycle = self.timings.get_turns_left_in_cycle(self._map.turn)
        launchPath = self.target_player_gather_path
        ...
        if self.target_player_gather_path.start.tile.army < 7 or self.target_player_gather_path.start.tile.army < fullFriendlyArmy / 5 + 1:
            return existingPlan
        ...
        launchValPerTurn = econVal / turns
        ...
        if launchValPerTurn > existingValPerTurn:
    ...
    def calculate_launch_path_values(self, launchPath, turnsLeftInCycle) -> typing.Tuple[int, int, int, int, int, int, int]:
        ...
        for tile in launchPath.tileList:
            ...
            if isFriendly:
                ...
                isTileTarget |= self._map.is_player_on_team_with(self.targetPlayer, self.territories.territoryMap[tile.x][tile.y]) and not tile.visible
                if isTileTarget:
                    enCaps += 1
                else:
                    neutCaps += 1
    ...
    def make_second_25_move(self) -> Move | None:
        ...
        leafMovesClosestToGen = list(sorted(self.leafMoves, key=lambda m: self.distance_from_general(m.dest)))
        # ignore the leafmoves closest to general
        leafMoves = leafMovesClosestToGen[6:]

        genOptionDistances = SearchUtils.build_distance_map(self._map, possibleGenTargets)
        ...
        for move in prioritizedLeaves:
            ...
            enDist = genOptionDistances[src.x][src.y]
            ...
            expandabilityPath = SearchUtils.breadth_first_dynamic_max(
                self._map,
                startTiles={src: ((0, 100, None, genOptionDistances[src.x][src.y]), 0)},
                valueFunc=lambda t, distTuple: (distTuple[0], 0 - distTuple[1]),
                priorityFunc=lambda t, distTuple: (distTuple[0] + 1, genOptionDistances[t.x][t.y], t, distTuple[1]) if (not self._map.is_tile_friendly(t) or t == src) and t not in negTiles and not t.isObstacle else None,
                skipFunc=lambda t, distTuple: False if distTuple[2] is None or genOptionDistances[t.x][t.y] < distTuple[3] else True,
                maxDepth=8,
    ...
    def get_city_preemptive_defense_move(self, defenseCriticalTileSet: typing.Set[Tile]) -> Move | None:
        ...
        if len(sketchyCities) > 0:
            ...
            if sketchyArmy > sketchyOutOfPlayThresh and self.sketchiest_potential_inbound_flank_path is not None:
                furthestCity = max(sketchyLargeArmyCities, key=lambda c: self.board_analysis.intergeneral_analysis.aMap[c.x][c.y])
                furthestDist = self.board_analysis.intergeneral_analysis.aMap[furthestCity.x][furthestCity.y]
                fogDist = self.board_analysis.intergeneral_analysis.aMap[self.sketchiest_potential_inbound_flank_path.tail.tile.x][self.sketchiest_potential_inbound_flank_path.tail.tile.y]
                ...
                if fogDist <= furthestDist:
                    ...
                    with self.perf_timer.begin_move_event('city preemptive flank defense get_flank_vision_defense_move'):
                        flankDefenseMove = self.get_flank_vision_defense_move(self.sketchiest_potential_inbound_flank_path, negs, atDist=furthestDist)
    ...
    def get_flank_vision_defense_move(self, flankThreatPath: Path, negativeTiles: typing.Set[Tile], atDist: int) -> Move | None:
        included = set()
        for tile in flankThreatPath.get_subsegment((flankThreatPath.length * 5) // 6).tileList:
            for adj in tile.adjacents:
                pathWay = self.board_analysis.intergeneral_analysis.pathWayLookupMatrix[adj]
                if pathWay is None or pathWay.distance < atDist:
                    continue
                if adj.isUndiscoveredObstacle or adj.isMountain:
                    continue
                included.add(adj)
        ...
        capture_first_priority_func, capture_first_value_func = self.get_capture_first_tree_move_prio_funcs()

        move = None
        offset = 0

        while move is None and offset < 15:
            move, valGathered, gatherTurns, gatherNodes = self.get_gather_to_target_tiles(
                [t for t in included],
                maxTime=0.001,
                gatherTurns=offset + (50 - self._map.turn) % 4,
                maximizeArmyGatheredPerTurn=True,
                targetArmy=0,
                leafMoveSelectionValueFunc=capture_first_value_func,
                leafMoveSelectionPriorityFunc=capture_first_priority_func,
                useTrueValueGathered=True,
                includeGatherTreeNodesThatGatherNegative=False,
                negativeSet=negativeTiles)

            playerArmyBaseline = int(self.player.standingArmy / self.player.tileCount)
            if valGathered / gatherTurns * 2 < playerArmyBaseline:
                logbook.info(f'iterating longer on get_flank_vision_defense_move due to poor valGathered{valGathered} / gatherTurns {gatherTurns} compared to {playerArmyBaseline}')
                move = None
            offset += 2
    ...
    def get_n_closest_team_tiles_near(self, nearTiles: typing.List[Tile], player: int, distance: int, limit: int, includeNeutral: bool = False) -> typing.List[Tile]:
        ...
        SearchUtils.breadth_first_foreach(self._map, nearTiles, distance, foreachFunc=nearbyTileAdder, skipFunc=lambda t: len(tiles) > limit)
    ...
    def try_get_cyclic_all_in_move(self, defenseCriticalTileSet) -> Move | None:
        winningEc = self.opponent_tracker.winning_on_economy(byRatio=1.25)
        winningTile = self.opponent_tracker.winning_on_tiles(byRatio=1.1)
        winningArmy = self.opponent_tracker.winning_on_army(byRatio=1.1)
        reason = ''
        if self.is_all_in_losing:
            reason = 'lose '
                ...
                if not self.is_all_in_losing:
                    ...
                    if enAttackPath is not None:
                        ...
                        if len(enTiles) > 5 and len(enTiles) > self.shortest_path_to_target_player.length // 2:
                            targets.extend(enAttackPath.tileList)

                with self.perf_timer.begin_move_event(f'{reason}gather cyclic {remainingTurns}'):
                    move, valueGathered, turnsUsed, gatherNodes = self.get_gather_to_target_tiles(targets, maxTime=0.1, gatherTurns=remainingTurns, negativeSet=negatives, useTrueValueGathered=False, includeGatherTreeNodesThatGatherNegative=True, maximizeArmyGatheredPerTurn=True)
    ...
    def try_get_enemy_territory_exploration_continuation_move(self, defenseCriticalTileSet: typing.Set[Tile]) -> Move | None:
        ...
        move = self._get_expansion_plan_exploration_move(armyCutoff)
    ...
    def _get_expansion_plan_exploration_move(self, armyCutoff: int) -> Move | None:
        move = None
        for path in self.expansion_plan.all_paths:
            ...
            if self.territories.territoryDistances[self.targetPlayer][path.start.tile] > 2:
                continue
    ...
    def render_intercept_plan(self, plan: ArmyInterception, colorIndex: int = 0):
        for tile, iw in plan.common_intercept_choke_widths.items():
            self.viewInfo.add_targeted_tile(tile, TargetStyle(((colorIndex + 1) % 9) + 1), radiusReduction=11 - colorIndex)
            self.viewInfo.bottomMidRightGridText[tile.x][tile.y] = f'iw{iw}'

        for dist, (val, path) in plan.intercept_options.items():
            logbook.info(f'intercept plan {plan.target_tile} dist {dist}: {val:.2f} {path}')
