...
class ThreatObj(object):
    def __init__(self, moveCount, threatValue, path, type, saveTile=None, armyAnalysis=None):
        ...
        self.threatValue: float = threatValue
        self.path: Path = path
...
class DangerAnalyzer(object):
    ...
    def getVisionThreat(self, general, depth, armies):
        ...
        for player in self.map.players:
            if not player.dead and (player.index != general.player) and len(self.playerTiles[player.index]) > 0 and \
                ...
                path = dest_breadth_first_target(self.map, general.adjacents, 0, 0.01, depth, None, player.index, False,
                                                 2)
                if path != None and (curThreat == None or path.length < curThreat.length or (
    ...
    def getFastestThreat(self, general, depth, armies):
        ...
        searchArmyAmount = -1.5
        for player in self.map.players:
            if not player.dead and (player.index != general.player) and player.index not in self.map.teammates and len(
                    self.playerTiles[player.index]) > 0 and self.map.players[player.index].tileCount > 10:
                path = dest_breadth_first_target(self.map, [general], searchArmyAmount, 0.05, depth, None, player.index,
                                                 False, 5)
                if path != None and (curThreat == None or path.length < curThreat.length or (
                        path.length == curThreat.length and path.value > curThreat.value)):
                    ...
                    altPath = dest_breadth_first_target(self.map, [general], searchArmyAmount, 0.05, path.length + 5,
                                                        None, player.index, False, 5, skipTiles=[lastTile])
        ...
        for armyTile in armies.keys():
            ...
            # if army.player != general.player:
                ...
                if path != None:
                    ...
                    if path.value > 0 and (
                            curThreat == None or path.length < curThreat.length or path.value > curThreat.value):
                        curThreat = path
